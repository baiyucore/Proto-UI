...
import { EVENT_REGISTRATION_CASES } from '@proto.ui/spec-fixtures/event/registration';
import { describe, expect, it } from 'vitest';

describe('spec fixtures: event registration', () => {
  it('stays aligned with T-EVENT-0001 and covered contracts', async () => {
    const workspace = await loadSpecWorkspaceFromDirectory(path.resolve(process.cwd(), 'spec'));
    expect(workspace.issues).toEqual([]);

    const testSpec = workspace.entities.find((entity) => entity.id === 'T-EVENT-0001');
    ...
    const fixtureSpecCases = new Set<string>(
      EVENT_REGISTRATION_CASES.map((testCase) => testCase.specCase)
    );

    for (const testCase of EVENT_REGISTRATION_CASES) {
      expect(
    ...
    const fixtureImplementation = testSpec?.implementations.find(
      (implementation) => implementation.id === 'event-registration-fixture'
    );

    expect(fixtureImplementation?.status).toBe('active');
    expect(fixtureImplementation?.path).toBe('packages/spec/fixtures/src/event/registration.ts');
